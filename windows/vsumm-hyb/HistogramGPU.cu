...
extern "C"
vector<HistogramGPU> callComputeHist(string frameDir, double &timeCut)
{
	cv::TickMeter timeLocal, time; 
	double timet = 0.0; timeCut = 0.0;
	...
	size_t totalSize = 0;// rows*cols*3*frameNames.size()*sizeof(unsigned char);
		...
		//if(result->getLength() > 5 && result->getResolution() == "1920")
		//	freeMem = freeMem / 5;


		cv::Mat imgref = cv::imread(frameNames[0]);
		int rows = imgref.rows;
		int cols = imgref.cols;

		int cont = 0;
		int frameInicial = 0;

		time.stop(); timeCut += time.getTimeSec();
		...
		while(cont < frameNames.size())
		{
			...
			{
				//if(imgref.cols == 1920 && atoi(splitStr[1].c_str()) >= 5 && framesQnt >= 300)
				//	break;
				//if(imgref.cols == 1280 && atoi(splitStr[1].c_str()) >= 20 && framesQnt >= 300)
				//	break;
