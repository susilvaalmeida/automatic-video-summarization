...
int main(int argc, char **argv) {
	...
	for(int i=0; i<(int)videoNames.size(); i++)
	{
		...
		//-----------------------CREATE DIRECTORIES------------------------------//
		string summaryDir = "summaryCPU-"+FileOperations::getSimpleName(videoNames[i])+"\\";
		...
		string frameDir = "framesCPU-"+FileOperations::getSimpleName(videoNames[i])+"\\";
		...
		DecoderCPU::decodeWithoutSave(videoNames[i],frameDir);
		timeLocal.stop();
