...
void ClusteringMULT::findClusterTH(int start, int end, double &delta, vector<vector<float> >& newClusters, vector<int>& newClusterSize)
{
...
void ClusteringMULT::kmeansTHR()
{
	...
	do
	{
		...
		int id = 0;
		int jump = (int)featuresM.size() / max_num_threads;
