...
int main(int argc, char **argv) {
	...
	for(int i=0; i<(int)videoNames.size(); i++)
	{
		...
		//-----------------------CREATE DIRECTORIES------------------------------//
		string frameDir = "framesOMP-"+FileOperations::getSimpleName(videoNames[i])+"\\";
		...
		DecoderMULT::decodeWithouSaveOMP(videoNames[i], frameDir);
		timeLocal.stop();
		result->setDecodeWithoutWrite(timeLocal.getTimeSec());
		...
		cout << "Processing video " << videoNames[i] << "..." << endl;
		//-----------------------CREATE DIRECTORIES------------------------------//
		frameDir = "framesTHR-"+FileOperations::getSimpleName(videoNames[i])+"\\";
		...
		timeLocal.reset(); timeLocal.start();
		DecoderMULT::decodeWithouSaveTHR(videoNames[i], frameDir);
		timeLocal.stop();
		result->setDecodeWithoutWrite(timeLocal.getTimeSec());
		...
		timeLocal.reset(); timeLocal.start();
		//------------------------VIDEO SEGMENTATION-----------------------------//
		DecoderMULT::saveFramesTHR(videoNames[i], frameDir);
		//-----------------------------------------------------------------------//
		timeLocal.stop();
		result->setDecode(timeLocal.getTimeSec());
		...
		sort(frameNamesT.begin(), frameNamesT.end());

		FeaturesMULT featT(frameNamesT, "thr");
		...
		for(int j=0; j<(int)finalKeyframes.size(); j++)
		{
			if(finalKeyframesT[j].getIdFrame() >= frameNamesT.size())
				continue;
			FileOperations::copyFile(frameNamesT[finalKeyframes[j].getIdFrame()], summaryDir);
		}
