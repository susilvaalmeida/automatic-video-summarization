...
#pragma omp parallel for
		...
		{
			if(i==max_num_threads-1)
				threadsFeatures(shots, histograms, idInicial, shots.size(),i);
			else
			{
...
void FeaturesMULT::filterFrames(string dir, string type)
{
	...
	int div = framesSummary.size() / max_num_threads;
	int idInicial = 0;
	int idFinal = div;
...
#pragma omp parallel for
		...
		{
			if(i==max_num_threads-1)
				threadFilter(framesSummary,idInicial,framesSummary.size(), dir);
			else
			{
