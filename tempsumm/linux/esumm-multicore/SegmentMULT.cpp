...
SegmentMULT::SegmentMULT()
{

}

void SegmentMULT::threadSegment(int idInicial, int idFinal)
{

	Mat frame1, frame2, grayframe1, grayframe2;
	for(int i=idInicial; i<idFinal-1; i++)
	{
		frame1 = imread(frames[i]);
		frame2 = imread(frames[i+1]);

		frameHistRGB(frame1, 256, i);
		frameHistRGB(frame2, 256, i+1);

		cosineDissimilarity(histograms[i], histograms[i+1], i);
		...
		if(i>0)
		{
			...
			else if(frame1.channels() == 1)
				grayframe1 = frame1;
			frameVariance(grayframe1, i);

		}
		if(frame2.channels() == 3)
			cvtColor(frame2, grayframe2, CV_RGB2GRAY);
		else if(frame2.channels() == 1)
			grayframe2 = frame2;

		frameVariance(grayframe2, i+1);
	}
}

SegmentMULT::SegmentMULT(string framesDir, string type)
{
	...
	dissimilarity.resize(frames.size()-1);
	...
	int div = frames.size() / max_num_threads;
	int idInicial = 0;
	int idFinal = div;
	...
	if(type == "thr")
	{
		...
		for(int i=0; i<max_num_threads; i++)
		{
			if(i==max_num_threads-1)
				vThreads.push_back(thread(&SegmentMULT::threadSegment, this, idInicial, frames.size()));
			else
			{
				vThreads.push_back(thread(&SegmentMULT::threadSegment, this, idInicial, idFinal));
				idInicial = idFinal;
...
#pragma omp parallel for
		...
		{
			if(i==max_num_threads-1)
				threadSegment(idInicial, frames.size());
			else
			{
				threadSegment(idInicial, idFinal);
				idInicial = idFinal;
				idFinal = idInicial + div;
			}
		}
	...
	convMat[1] = -1.0;
	vector<double> convEffects = MathOperationsMULT::conv(effects, convMat, "same");
...
void SegmentMULT::frameHistRGB(Mat frame, int bins, int indice)
{
	vector<int> hist(bins*3);
	for(int i=0; i<frame.rows; i++){
		for(int j=0; j<frame.cols; j++){
			hist[(int)(frame.at<cv::Vec3b>(i,j)[0])+2*bins]++;
			hist[(int)(frame.at<cv::Vec3b>(i,j)[1])+bins]++;
			hist[(int)(frame.at<cv::Vec3b>(i,j)[2])]++;
		}
	}
	...
	if(indice == -1)
		...
		this->histograms[indice] = hist;
}

void SegmentMULT::frameVariance(Mat grayframe, int indice)
{
	if(grayframe.channels() == 1)
	{
		...
		if(indice == -1)
			...
			variances[indice] = variance;
	}
}

void SegmentMULT::cosineDissimilarity(vector<int> hist1, vector<int> hist2, int indice)
{
	...
	if(indice == -1)
		...
		dissimilarity[indice] = 1-similarity;
}


vector<double> SegmentMULT::filterDissimilarity(int qntNeighbors)
{
...
vector<vector<int>> SegmentMULT::dissolveDetectionIndexes()
{
	double c = 0.7;
	...
	mvar = MathOperationsMULT::medianfilter(mvar,5);
	...
	vector<double> v1 = MathOperationsMULT::conv(v,matConv1,"same");

	vector<double> matConv2(3,1); 
	matConv2[1]=-2;

	vector<double> v2 = MathOperationsMULT::conv(v,matConv2,"same");
...
vector<vector<int>> SegmentMULT::mergeIntervals(vector<vector<int>> intervals, int dist)
{
	vector<int> ni;
	vector<int> ne;
...
Eigen::SparseMatrix<double> SegmentMULT::createSparse(vector<vector<int>> mat, vector<int> diags, int rows, int cols)
{
...
Eigen::SparseMatrix<double> SegmentMULT::createSparse(vector<int> mat, int diags, int rows, int cols)
{
	Eigen::SparseMatrix<double, Eigen::ColMajor> sparse(rows,cols);
	sparse.reserve(Eigen::VectorXi::Constant(cols,3));
...
vector<double> SegmentMULT::smoothSpline(vector<double> mvar, double parameter)
{
	int n = mvar.size();
	...
	vector<double> firstDiff = MathOperationsMULT::diff(mvar);
	for(int i=0; i<(int)firstDiff.size(); i++)
		firstDiff[i] /= dx[i];

	vector<double> secondDiff = MathOperationsMULT::diff(firstDiff);
	...
	Eigen::VectorXd diffConc = MathOperationsMULT::diff(concat);
	...
	Eigen::VectorXd diffConc2 = MathOperationsMULT::diff(concat2);
...
vector<double> SegmentMULT::findEffects()
{
