...
vector<double> MathOperationsCPU::conv(vector<double>& mA, vector<double>& mB, string shape){
...
vector<double> MathOperationsCPU::medianfilter(vector<double>& vetor, int W){
...
float MathOperationsCPU::dist(vector<float>& data, vector<float>& centers)
{
	float soma = 0.0;
...
vector<float> MathOperationsCPU::dist(vector<vector<int>>& data, vector<float>& centers)
{
...
vector<float> MathOperationsCPU::dist(vector<vector<float>>& data, vector<float>& centers)
{
	vector<float> distances;
	for(int i=0; i<(int)data.size(); i++)
	{
...
void MathOperationsCPU::dist(vector<vector<float>>& data, vector<float>& centers, vector<float>& dist)
{
	for(int i=0; i<(int)data.size(); i++)
	{
		float soma = 0;
...
vector<double> MathOperationsCPU::diff(const vector<double>& in)
{
...
Eigen::VectorXd MathOperationsCPU::diff(Eigen::VectorXd vetor)
{
...
void MathOperationsCPU::meanCols(vector<vector<float>>& data, vector<float>& means)
{
...
vector<float> MathOperationsCPU::histHSV(Mat frame, int bins)
{
	cv::Mat hsvImg;
...
vector<float> MathOperationsCPU::histRGB(Mat frame, int bins)
{
...
float MathOperationsCPU::variance(Mat grayframe)
{
...
float MathOperationsCPU::correlation(vector<float>& img1, vector<float>& img2)
{
