...
void computeHistVarDiss(unsigned char* images, int frameInicial, int qntFrames, int rows, int cols, vector<double>& variances, vector<double>& dissimilarity, vector<vector<int> >& histograms)
{
	...
	gray_kernel<<<dimGrid,dimBlock,0>>>(gpu_in, gpu_out, rows, cols, qntFrames);
	...
	mean_variance_kernel<<<tt,32,0>>>(gpu_out, var_gpu_out, rows*cols, qntFrames);
	...
	//compute histograms
	histograma_basic_kernel<<<dimGrid,dimBlock,0>>>(gpu_in, histGPU, rows, cols, qntFrames);
	...
	cosineDissimilarity_kernel<<<tt,32,0>>>(histGPU, dissGPU, 256*3, qntFrames);
...
void callComputations(string framesDir, vector<double>& variances, vector<double>& dissimilarity, vector<vector<int> >& histograms)
{
	...
	while(cont < frames.size())
	{
		cv::Mat concat;
		int framesQnt = 0;
		while(totalSize < freeMem && cont < frames.size())
		{
