...
vector<double> MathOperations::conv(vector<double>& mA, vector<double>& mB, string shape){
...
vector<double> MathOperations::medianfilter(vector<double>& vetor, int W){
...
float MathOperations::dist(vector<float>& data, vector<float>& centers)
{
	float soma = 0.0;
...
vector<float> MathOperations::dist(vector<vector<int> >& data, vector<float>& centers)
{
...
vector<float> MathOperations::dist(vector<vector<float> >& data, vector<float>& centers)
{
	vector<float> distances;
	for(int i=0; i<(int)data.size(); i++)
	{
...
void MathOperations::dist(vector<vector<float> >& data, vector<float>& centers, vector<float>& dist)
{
	for(int i=0; i<(int)data.size(); i++)
	{
		float soma = 0;
...
vector<double> MathOperations::diff(const vector<double>& in)
{
...
void MathOperations::meanCols(vector<vector<float> >& data, vector<float>& means)
{
...
vector<float> MathOperations::histHSV(Mat frame, int bins)
{
	cv::Mat hsvImg;
...
vector<float> MathOperations::histRGB(Mat frame, int bins)
{
...
float MathOperations::variance(Mat grayframe)
{
	if(grayframe.channels() == 1)
	{
		Scalar mean, stdDev;
		meanStdDev(grayframe, mean, stdDev);
		float variance = (float)pow(stdDev[0],2);
		return variance;
	}
	return -1;
}

float MathOperations::correlation(vector<float>& img1, vector<float>& img2)
{
	...
	float a = (float)std::accumulate(mult.begin(), mult.end(), 0.0);
	...
	float b = sqrt((float)std::accumulate(exp1.begin(), exp1.end(),0.0)) * 
		sqrt((float)std::accumulate(exp2.begin(), exp2.end(),0.0));
