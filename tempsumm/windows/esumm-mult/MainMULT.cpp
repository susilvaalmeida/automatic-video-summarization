...
int main(int argc, char **argv) {
	...
	for(int i=0; i<(int)videoNames.size(); i++)
	{
		...
		string dname(argv[2]);
		//-----------------------CREATE DIRECTORIES------------------------------//
		string frameDir = "framesMULT-"+dname+"-"+FileOperations::getSimpleName(videoNames[i])+"\\";
		string featuresDir = "featuresMULT-"+dname+"-"+FileOperations::getSimpleName(videoNames[i])+"\\";

		string summaryDir = "summaryMULT-"+dname+"-"+FileOperations::getSimpleName(videoNames[i])+"\\";
		...
		cout << "Processing video " << videoNames[i] << "..." << endl;
		//-----------------------CREATE DIRECTORIES------------------------------//
		frameDir = "framesMULTT-"+dname+"-"+FileOperations::getSimpleName(videoNames[i])+"\\";
		featuresDir = "featuresMULTT-"+dname+"-"+FileOperations::getSimpleName(videoNames[i])+"\\";
		summaryDir = "summaryMULTT-"+dname+"-"+FileOperations::getSimpleName(videoNames[i])+"\\";
		...
		for(int ki=0; ki<(int)featMULT1.representativeFrames.size(); ki++)
		{
			for(int j=0; j<(int)featMULT1.representativeFrames[ki].size(); j++)
			{
				vetorRepresFrames.push_back(featMULT1.representativeFrames[ki][j]);
			}
		}
