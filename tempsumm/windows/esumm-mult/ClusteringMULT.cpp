...
{
	string command = "copy " + featDir + "*.feat " + featDir + "allFeats.feats";
	system(command.c_str());

	dataLBG.clear();
	FileOperations::readMatFile(dataLBG, featDir + "allFeats.feats");
}
		...
		string cutFactor, string maxIterations, string numSplits, string maxCenters)
{
	...
	string comando;
	//system("pwd");
	comando = "kmeans.exe makeuni in matlabintro" + ID +".unit.ds";
	system(comando.c_str());

	comando = "kmeans.exe kmeans -k "+parameterK+" -method blacklist -max_leaf_size "+
...
void ClusteringMULT::findCodeWords(int start, int end, vector<vector<float>>& data, vector<vector<float>>& clusters, vector<int>& indexes)
{
	for(int i=start; i<end; i++)
	{
		int nearest = findNearestCluster(data[i], clusters, (int)clusters.size());
		indexes[i] = nearest;
	}
}
...
void ClusteringMULT::callLBG(int qntCenters, string type)
{
	...
	for(int countSplits = 0; countSplits < ceil(log10(qntCenters)/log10(2.0)); countSplits++)
	{
		...
		while(diffDistortion > epsilon)
		{
			...
			if(type == "omp")
			{
				int framesPerThread = (int)this->dataLBG.size() / max_num_threads;
...
#pragma omp parallel for
			...
			{
				int id = 0;
				int jump = (int)this->dataLBG.size() / max_num_threads;
...
void ClusteringMULT::codeWordsHistogram(int bins, string featDir, string type)
{
	...
	for(int i=0; i<(int)feats.size(); i++)
	{
		...
		if(type == "omp")
		{
			int framesPerThread = (int)features.size() / max_num_threads;
...
#pragma omp parallel for
		...
		{
			int id = 0;
			int jump = (int)features.size() / max_num_threads;
