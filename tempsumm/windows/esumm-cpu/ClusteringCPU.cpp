...
{
	string command = "copy " + featDir + "*.feat " + featDir + "allFeats.feat";
		...
		string cutFactor, string maxIterations, string numSplits, string maxCenters)
{
	...
	string comando;
	//system("pwd");
	comando = "kmeans.exe makeuni in matlabintro.unit.ds";
...
void ClusteringCPU::codeWordsHistogram(int bins, string featDir)
{
	...
	for(int i=0; i<(int)feats.size(); i++)
	{
		...
		{
			int nearest = findNearestCluster(features[j], centersLBG, (int)centersLBG.size());
			index[j] = nearest;
		}
