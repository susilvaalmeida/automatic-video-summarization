...
FeaturesCPU::FeaturesCPU(string descript, string framesDir, string featDir, vector<vector<int>>& shots, vector<vector<int>>& histograms)
{
...
Mat FeaturesCPU::calcHog(Mat img)
{
	cv::HOGDescriptor hogCPU(Size(16,16), Size(16,16), Size(8,8), Size(8,8), 9);
	vector<float> descriptors;
	vector<Point> locs;

	hogCPU.compute(img, descriptors, Size(8,8), Size(), locs);
...
Mat FeaturesCPU::calcSift(Mat img)
{
	Mat siftMat;
...
Mat FeaturesCPU::calcSurf(Mat img)
{
	Mat surfMat;
...
void FeaturesCPU::filterFrames(string dir)
{
	//double th = 0.75;
	double thvar = 700;
	...
	for(int i=0; i<(int)framesSummary.size()-1; i++)
	{
		...
		vector<float> himPivot = MathOperationsCPU::histHSV(imPivot, 16);
		for(int j=0; j<(int)himPivot.size(); j++)
			himPivot[j] /= imPivot.rows*imPivot.cols;

		vector<float> himNext = MathOperationsCPU::histHSV(nextImg, 16);
		...
		vector<float> him1 = MathOperationsCPU::histRGB(imPivot, 256);
		for(int j=0; j<(int)him1.size(); j++)
			him1[j] /= imPivot.rows*imPivot.cols;

		vector<float> him2 = MathOperationsCPU::histRGB(nextImg, 256);
		for(int j=0; j<(int)him2.size(); j++)
			him2[j] /= nextImg.rows*nextImg.cols;

		float corrHisto = MathOperationsCPU::correlation(him1, him2);
		...
		float corrIma = MathOperationsCPU::correlation(im1V, im2V);
	...
	for(int i=0; i<(int)framesSummary.size(); i++)
	{
		...
		float var = MathOperationsCPU::variance(im1);
